...
int wifi_ini_convert(void)
{
	...
	memcpy(sec1_table, (s8_t *)sec1, sizeof(struct wifi_conf_sec1_t));
	memcpy(sec2_table, (s8_t *)sec2, sizeof(struct wifi_conf_sec2_t));
	int sec1_len = sizeof(struct wifi_conf_sec1_t);
	...
	char *pre_header = "#ifndef __WIFI_RF_H__\n#define __WIFI_RF_H__\n";
	fprintf(fp, "%s", pre_header);

	char *header_files = "#include <zephyr/types.h>\n\n";
	fprintf(fp, "%s", header_files);
	...
	char *sec2_pre = "const u8_t sec2_table[] = {\n";
	fprintf(fp, "%s", sec2_pre);
	len = 1;
	for (i = 0; i < sec2_len; i++) {
		fprintf(fp, "0x%.2x", sec2_table[i]);
